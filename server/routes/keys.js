...
router.post('/', requireAuth, async (req, res) => {
  ...
  const result = db.prepare(
    'UPDATE users SET public_key = ?, encrypted_private_key = ? WHERE id = ?'
  ).run(public_key, encrypted_private_key, req.user.id);

  console.log(`[keys] user ${req.user.id} saved, changes:`, result.changes);
  res.json({ ok: true });
});
...
router.post('/char-pos', requireAuth, async (req, res) => {
  const db = await dbPromise;
  const { x, y } = req.body || {};
