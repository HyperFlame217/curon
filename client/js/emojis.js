...
function _syncPanelPosition(panelId, anchorId, panelW) {
  ...
  const iRect = inputWin.getBoundingClientRect();
...
function _initLayoutListeners() {
  if (_layoutListenersInit) return;
  _layoutListenersInit = true;
  const sync = () => {
    _syncPanelPosition('gif-panel', 'btn-gif', 340);
    _syncPanelPosition('emoji-panel', 'btn-emoji', 320);
    _syncPanelPosition('emoji-autocomplete', 'ifield', 220);
  };
...
async function sendGif(gif) {
  ...
  // Send GIF as plaintext content (Hybrid E2EE Transition)
...
async function sendMediaMessage(file) {
  ...
  // Send media metadata as plaintext content
  wsSend(WS_EV.C_MESSAGE_SEND, { content: media.mime_type, media_id: media.id, reply_to_id: REPLY_STATE.active ? REPLY_STATE.msgId : null });
  cancelReply();
  showToast('');
}
...
function buildAudioPlayer(src) {
  const wrap = document.createElement('div');
  wrap.className = 'ab';
  wrap.style.cssText = 'display:flex;align-items:center;gap:6px;padding:4px 8px;background:var(--color-base);border:2px solid var(--color-dark);box-shadow:2px 2px 0 var(--color-dark);min-width:220px;max-width:350px;max-height:60px;';
  ...
  info.innerHTML = `
    ...
    <div class="abar" style="height:4px;background:var(--color-soft);border:1px solid var(--color-dark);cursor:pointer;position:relative;">
      <div class="abar-fill" style="position:absolute;left:0;top:0;height:100%;width:0%;background:var(--color-tertiary);"></div>
    </div>
